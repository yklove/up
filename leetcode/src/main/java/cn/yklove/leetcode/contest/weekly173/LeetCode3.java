...
public class LeetCode3 {
    Map<Integer, List<Bian>> map = new HashMap<>();
    Set<Integer> countList = new HashSet<>();
    Set<Integer> exculed = new HashSet<>();
    ...
    public int findTheCity(int n, int[][] edges, int distanceThreshold) {
        ...
        for (int i = 0; i < edges.length; i++) {
            map.get(edges[i][0]).add(new Bian(edges[i][0], edges[i][1], edges[i][2]));
            map.get(edges[i][1]).add(new Bian(edges[i][1], edges[i][0], edges[i][2]));
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (countList.size() < min) {
                min = countList.size();
                minN = i;
            } else if (countList.size() == min) {
                if (i > minN) {
                    minN = i;
                }
            }
    ...
    private void f(Integer n, int distanceThreshold) {
        for (Bian bian : map.get(n)) {
            if (distanceThreshold - bian.weight >= 0 && !exculed.contains(bian.to)) {
                countList.add(bian.to);
                exculed.add(bian.to);
                f(bian.to, distanceThreshold - bian.weight);
                exculed.remove(bian.to);
            }
        }
    }
    ...
    static class Bian {
        ...
        public Bian(int from, int to, int weight) {
            this.from = from;
            this.to = to;
            this.weight = weight;
        }
